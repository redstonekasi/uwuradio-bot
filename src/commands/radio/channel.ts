...
export default new Command({
  ...
  async handler(interaction) {
    ...
    if (
      !client.config.sudoers.includes(member.id) &&
    ...
    if (!channel) {
        ...
        return void interaction.editReply({
          embeds: [
            createStatusEmbed({
              type: "error",
              description:
                "I'm not in any channel in this guild right now - specify `channel` to make me join one",
            }),
          ],
        });
      ...
      client.config.channels[guild.id] = channel.id;
      leaveChannel(guild);
      if (channel.members.size !== 0) joinChannel(channel);
